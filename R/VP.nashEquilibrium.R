...
#' @param pr_range_mat A matrix (2 columns) containing the upper and lower boundaries for the
#' optimisation.
...
#' 1st column: sequencial index. 2nd column: indicator for groups.
#' 3rd column: indicator for line pricing
#' @param line_price A boolean variable telling if line pricing should be applied
#' @param freeze A vector inculding the SKU numbers that should be excluded from the
#' optimisation. default = \code{NULL}
...
#' @param constraints A vector indicating the allowed increase/decrease in share,
#' profit or revenue (in percentage points). If \code{NULL} (default) no constraints will be applied.
#' @param optKPI A string indicating the KPI to be optimized (profit, revenue, share).
#' default \code{"profit"}.
...
#' }
#'
#' @export VP.nashEquilibrium

VP.nashEquilibrium <- function(utils, simSKUs, startingPrices, prices, pr_range_mat = NULL, nlev, costs,
  ...
  # costs <- diag(costs)
  if (is.null(pr_range_mat)) pr_range_mat <-  t(apply(prices, 1, range))
  ...
  tol <- .01
  maxIter <- 20
  base_profit <- VP.computeProfit(utils, prices, simPrices = startingPrices, simSKUs,
      ...
      oldPrices <- prices_used
        ...
        for (i in simSKUs) {
          ...
          if (any(test)) {
            ...
            cp <- function(p) {
              prices_used[i] <- p
              sum(VP.computeProfit(utils, prices, simPrices = prices_used, simSKUs,
            ...
            if (!is.null(constraints)) {
              ...
              check_share <- unlist(lapply(split(VP.computeShares(utils, prices, prices_used,
                                                                         simSKUs, nlev, weight,
      ...
      if (!line_price) {
        ...
        for (i in simSKUs) {
          ...
          if (any(test)) {
            ...
            cs <- function(p) {
              prices_used[i] <- p
              sum(VP.computeShares(utils, prices, simPrices = prices_used, simSKUs,
            ...
            if (!is.null(constraints)){
              # Constraints: added 19.10.16 (rm)
              check_Profit <- unlist(lapply(split(VP.computeProfit(utils, prices, prices_used,
                                                                   simSKUs, nlev, weight, none, iaw, FC,
                                                                   costs)$profit /
                                                    sum(VP.computeProfit(utils, prices, prices_used,
                                                                         simSKUs, nlev, weight, none,
                                                                         iaw, FC, costs)$profit),
                                                  split_factor), sum))
        ...
        for (i in simSKUs) {
          ...
          if (any(test)) {
            ...
            if (!is.null(constraints)) {
              # Constraints: added 19.10.16 (rm)
              check_Profit <- unlist(lapply(split(VP.computeProfit(utils, prices, prices_used, simSKUs,
