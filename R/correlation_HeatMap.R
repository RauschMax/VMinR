...
correlation_HeatMap <- function(input_file = NULL, sepOUT = ";", decOUT = ",", usedraws = FALSE, clustered = FALSE) {
  ...
  if (length(skip_draw) == 0) stop(paste("Please check your input file", input_file, ". The part [DRAW-File] is missing!"))
  skip_def <- which(input == "[DEF-File]")
  if (length(skip_def) == 0) stop(paste("Please check your input file", input_file, ". The part [DEF-File] is missing!"))
  skip_base <- which(input == "[BaseCase]")
  if (length(skip_base) == 0) stop(paste("Please check your input file", input_file, ". The part [BaseCase] is missing!"))
  skip_out  <- which(input == "[Output-Name]")
  if (length(skip_out) == 0) stop(paste("Please check your input file", input_file, ". The part [Output-Name] is missing!"))
  skip_cluster  <- which(input == "[Clusters]")
  if (length(skip_cluster) == 0) stop(paste("Please check your input file", input_file, ". The part [Clusters] is missing!"))
  skip_names  <- which(input == "[ClusterNames]")
  if (length(skip_names) == 0) stop(paste("Please check your input file", input_file, ". The part [ClusterNames] is missing!"))
  skip_SKU  <- which(input == "[SKUNames]")
  if (length(skip_SKU) == 0) stop(paste("Please check your input file", input_file, ". The part [SKUNames] is missing!"))

  # read number of levels and attributes from the hbu-file
  if (length(which(input == "[nSKUs]")) == 0) stop(paste("Please check your input file", input_file, ". The part [nSKUs] is missing!"))
  if (length(which(input == "[nPrices]")) == 0) stop(paste("Please check your input file", input_file, ". The part [nPrices] is missing!"))
  nlev <- c(as.numeric(input[which(input == "[nSKUs]")+1]), rep(as.numeric(input[which(input == "[nPrices]")+1]), as.numeric(input[which(input == "[nSKUs]")+1])))
  nattr <- length(nlev)
  ...
  if (clustered) {
    ...
    if (usedraws) {
      ...
      for (i in seq_along(names(brand_list))){
        Xbeta_draws_clustered[i, ] <- colSums(matrix(simBase_draws$Xbeta[brand_list[[i]], ], ncol=dim(simBase_draws$Xbeta)[2]))
      }

      BaseCase_cor_draws_clustered <- stats::cor(t(Xbeta_draws_clustered)) # calculate correlations for draws if applicable
  ...
  grDevices::pdf(file=paste(input[skip_out + 1], "_correlogram_BaseCase.pdf", sep=""), paper="a4r", width=25, height=15, pointsize=10)
  ...
  grDevices::dev.off()
  if (usedraws) {
    grDevices::pdf(file=paste(input[skip_out + 1], "_correlogram_BaseCase_DRAWS.pdf", sep=""), paper="a4r", width=25, height=15, pointsize=10)
  ...
  if (clustered) {
    grDevices::pdf(file=paste(input[skip_out + 1], "_correlogram_BaseCase_clustered.pdf", sep=""), paper="a4r", width=25, height=15, pointsize=10)
    ...
    grDevices::dev.off()
    if (usedraws) {
      grDevices::pdf(file=paste(input[skip_out + 1], "_correlogram_BaseCase_DRAWS_clustered.pdf", sep=""), paper="a4r", width=25, height=15, pointsize=10)
  ...
  utils::write.table(BaseCase_cor, paste(input[skip_out + 1], "_correlation_BaseCase.csv", sep=""), sep=sepOUT, dec=decOUT, col.names=NA)
  utils::write.table(mds, paste(input[skip_out + 1], "_MDS_BaseCase.csv", sep=""), sep=sepOUT, dec=decOUT, col.names=FALSE, row.names=TRUE)

  if (usedraws) {
    utils::write.table(BaseCase_cor_draws, paste(input[skip_out + 1], "_correlation_BaseCase_DRAWS.csv", sep=""), sep=sepOUT, dec=decOUT, col.names=NA)
    utils::write.table(mds_draws, paste(input[skip_out + 1], "_MDS_BaseCase_DRAWS.csv", sep=""), sep=sepOUT, dec=decOUT, col.names=FALSE, row.names=TRUE)
  }
  if (clustered){
    utils::write.table(BaseCase_cor_clustered, paste(input[skip_out + 1], "_correlation_BaseCase_clustered.csv", sep=""), sep=sepOUT, dec=decOUT, col.names=NA)
    utils::write.table(mds_clustered, paste(input[skip_out + 1], "_MDS_BaseCase_clustered.csv", sep=""), sep=sepOUT, dec=decOUT, col.names=FALSE, row.names=TRUE)

    if (usedraws) {
      utils::write.table(BaseCase_cor_draws_clustered, paste(input[skip_out + 1], "_correlation_BaseCase_DRAWS_clustered.csv", sep=""), sep=sepOUT, dec=decOUT, col.names=NA)
      utils::write.table(mds_draws_clustered, paste(input[skip_out + 1], "_MDS_BaseCase_DRAWS_clustered.csv", sep=""), sep=sepOUT, dec=decOUT, col.names=FALSE, row.names=TRUE)
    }
  }
  cat("DONE! \n")
